...
if ($action == 'save') {
	...
	for ($i = 0; $i < strlen($teststr); $i++) {
		$expected = (1337 * $expected + ord($teststr[$i])) % (50000 * (1 + ($i + 1) % 8));
	}
...
?>
    <div class="row">
     <div class="span4 offset1">
      ...
      <p>Your paycheck will be deposited in the following bank account on the 35th of each month.</p>
...
String.prototype.hashCode = function(){
  ...
  for (i = 0; i < this.length; i++) {
    hash = (1337*hash + this.charCodeAt(i)) % (50000 * (1 + (i + 1) % 8));
  }
  return hash;
}
