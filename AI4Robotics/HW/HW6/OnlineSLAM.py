...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # Set dimension of Omega
    # We only need the latest position
    ...
    # process the data
    # For each data item, grow the matrix, incorporate the new
    # data, and compress
    for k in range(len(data)): 
    
        # The index of the robot pose in the matrix/vector will
        # always be 0 (for x) and 1 (for y)
        n = 0
        ...
        # Expand the matrix by one motion (i.e., 2)
        # Create list for expanding matrix
        # [ 0, 1, 4, 5, ... ]
        list = []
        for i in range(dim):
           if i > 1:
        ...
        A = Omega.take([0, 1], list)
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])
        
        Oprime = Omega.take(list, list) 
        Xprime = Xi.take(list, [0])
        
        Omega = Oprime - A.transpose()*B.inverse()*A
        Xi    = Xprime - A.transpose()*B.inverse()*C
