...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        # 1) robot is in turn on the left
        # We know the centers of the two (half)circles so we can calculate the desired x
        # position given the y position
        if self.x <= radius:
            cte = sqrt((self.x - radius)**2 + (self.y - radius)**2) - radius
        
        # 2) Robot is in turn on the right
        elif self.x >= 3*radius:
            cte = sqrt((self.x - 3.0*radius)**2 + (self.y - radius)**2) - radius
        
        # Note that the way I ordered the if-else statements I can just check
        # the y value for the two straight aways
        
        # 3) Robot is in upper straight away
        elif self.y > radius:
            cte = self.y - 2.0*radius
            
        # 4) Robot is in lower straight away
        elif self.y < radius:
            # Moving left, so signs are reversed
            cte = -self.y     
