...
def estimate_next_pos(measurement, OTHER = None):
    ...
    # If OTHER is None, we have nothing to go off of (This is probably the 
    # first time it is called.) So just return the measurement for both
    if OTHER == None:
        xy_estimate = measurement
        OTHER = measurement, 0.0
    else:
        # The distance moved should always be the same
        d = distance_between(OTHER[0], measurement)
        
        # But since it is moving in a circle, we can't just add the distance        
        # (If it were moving in a straight line we could)
        # We have to determine the heading of the robot
        # I am going to keep track of the robot's last heading in OTHER
        # along with the last position
        
        # I will find the difference between the current heading and the previous
        # heading. Then, since this difference should be the same,
        # I can estimate the next position by using the distance
        # moved each time step with the estimate of the next heading
        x_diff = measurement[0] - OTHER[0][0]
        y_diff = measurement[1] - OTHER[0][1]
        h_est = atan2(y_diff, x_diff)
        
        x_estimate = measurement[0] + d*cos(h_est + (h_est-OTHER[1]))
        y_estimate = measurement[1] + d*sin(h_est + (h_est-OTHER[1]))
        
        xy_estimate = [x_estimate, y_estimate]
        
        OTHER = measurement, h_est
