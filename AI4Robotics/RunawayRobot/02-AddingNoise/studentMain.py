...
def kalman_filter(measurement, OTHER, P):
    ...
    I = matrix([[1.,0.,0.],
                [0., 1.,0.], 
                [0.,0.,1.]])
    # With this motion model, the state transition matrix is constant
    F = matrix([[1., 0., 0.],
                [0., 1., 0.],
                [0., 0., 1.]])
    # So is the measurement function. We observe x and y, but not heading 
    H = matrix([[1., 0., 0.],
                [0., 1., 0.]])
    # I believe measurement uncertainty is as well. Not sure how
    # to come up with the values here though. Seems like they should be
    # based off of those from the robot
    ...
    print d, (h_est-OTHER[1])
    
    # Generate estimate
    x_estimate = estimate.value[0][0] + d*cos(h_est + (h_est-OTHER[1]))
    y_estimate = estimate.value[1][0] + d*sin(h_est + (h_est-OTHER[1]))        
    estimate = matrix([[x_estimate], 
                       [y_estimate], 
                       [h_est]])
