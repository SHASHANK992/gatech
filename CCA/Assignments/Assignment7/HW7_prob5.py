...
def rowStrategy(A):
    ...
    # create variables
    prob = var(range(0,m), 'prob')
    w = var(name='game_value', bounds=(None,None))
    ...
    # constraints
    st(
        sum(prob[i] for i in range(0,m)) == 1.0
    )
    
    st(
        sum(float(A[i][j])*prob[i] for i in range(0,m)) >= w for j in range(0,n)
    )
    ...
    #print >> sys.stderr, A
    #print >> sys.stderr, np.array([float(prob[i].primal) for i in range(0,m) ], dtype=float)
    
    return np.array([float(prob[i].primal) for i in range(0,m) ], dtype=float)
